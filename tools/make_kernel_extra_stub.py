...
def section_header(name: int, sh_type: int, flags: int, addr: int, offset: int,
...
def build_stub() -> bytes:
    ...
    sh_name = {name: shstr.index(name.encode("ascii")) for name in [".shstrtab", ".dynamic", ".scenid", ".dynstr", ".dynsym"]}

    dynstr = bytearray(b"\0")
    offsets: dict[str, int] = {"": 0}
    symbol_offsets = [add_string(dynstr, offsets, name) for name in STUB["symbols"]]
    soname_offset = add_string(dynstr, offsets, STUB["soname"])
    module_offset = add_string(dynstr, offsets, STUB["module"])
    library_offset = add_string(dynstr, offsets, STUB["library"])
    ...
    for offset, name in zip(symbol_offsets, STUB["symbols"]):
        scenid += nid_bytes(name)
        dynsym += struct.pack("<IBBHQQ", offset, (STB_GLOBAL << 4) | STT_FUNC, 0, 0, 0, 0)
    ...
    ident += b"\0" * 7
    struct.pack_into("<16sHHIQQQIHHHHHH", out, 0, bytes(ident), ET_SCE_STUBLIB, EM_X86_64, 1,
                     0, 0, shoff, 0, 64, 0, 0, 64, len(headers), 1)
    return bytes(out)
